...
const { characters, pageInfo, changePage, itemsPerPage, currentPage, filterByName, currentFilter, loading } = useCharacters()
...
<template>
  ...
  <template v-else-if="characters.length > 0">
    <main class="w-full flex gap-4 flex-wrap justify-center p-8">
      <RouterLink v-for="character in characters" :key="character.id" :to="{ name: 'character', params: { id: character.id }}">
        <Character  v-bind="character" />
      </RouterLink>
    </main>
    <footer class="p-8">
      <Pagination v-slot="{ page }" :items-per-page="itemsPerPage" :total="pageInfo?.count" show-edges :sibling-count="1" :default-page="currentPage">
        <PaginationContent v-slot="{ items }" class="flex items-center gap-1">
          ...
          <template v-for="(item, index) in items">
            <PaginationItem v-if="item.type === 'page'" :key="index" :value="item.value" as-child>
              <Button @click="changePage(item.value)" class="w-10 h-10 p-0 cursor-pointer" :variant="item.value === page ? 'default' : 'outline'">
                {{ item.value }}
              </Button>
            </PaginationItem>
